...
- `gar.sh`: Google Artifact Registry and Container Registry authentication
  and lookup
- `ecr.sh`: Amazon ECR authentication and lookup
- `registries.sh`: registry classification and dispatch

To add registry-specific support, source its module before `registries.sh`,
classify its repository host in `registry_classify`, and add it to the
direct-tag and reverse-lookup dispatch functions. A direct tag lookup returns
the digest on stdout with status 0, status 1 when the tag does not exist, or
status 2 when the lookup fails. An exhaustive lookup populates `registry_tags`
and may also populate `registry_lookup_status` and `registry_metadata` for
registry-specific output.
