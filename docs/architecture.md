...
rows require an earlier `LOOKUP_DENIED`. A denial belongs to one mechanism: it
unlocks eligible credentialed rows without suppressing independent public
mechanisms. `if-required` exhausts applicable public rows before credentials,
...
| **4. Try the public native OCI path** | Obtain an anonymous repository-scoped token from `auth.docker.io`, list tags from `registry-1.docker.io`, and inspect candidate manifests. This is more request-intensive than Hub metadata but does not require a PAT. | Use when the Packages probe is unavailable or not permitted, or after any measured continuation is unavailable or denied. | Compatibility fallback when ACR metadata is unavailable or denied; success still depends on the registry enabling anonymous access. | Compatibility fallback when the GCR manifest-map response is unavailable or unusable. | List tags and inspect candidate manifests. This also verifies a non-authoritative GAR API miss. | Authentication probe only: the expected denial unlocks credentialed paths under `if-required` or after the signed API is unavailable. | Normal fallback when the signed API is unavailable or credentials are not permitted. ECR Public currently issues anonymous repository tokens and serves tag lists, although AWS does not document tag-list support. | Primary algorithm: list tags and inspect candidate manifests. |
| **5. Try provider credentials**<br>After `LOOKUP_DENIED`, or immediately under `require` | Exchange `DOCKER_HUB_USERNAME` and `DOCKER_HUB_PAT` for a short-lived Hub token, then resume the faster paginated Hub API scan. A PAT is not required for the native OCI alternative. | Search GitHub Packages pages using existing `gh` credentials with `read:packages`. | Ask Azure CLI for the digest-addressed manifest metadata. | Obtain a short-lived Google token and repeat the GCR manifest-map request. | Obtain a Google token and retry `dockerImages.get`. Its 404 remains non-authoritative. | Call signed `DescribeImages`, unless the same API was already attempted. | Call the signed ECR Public API, unless it was already attempted. | N/A |
...
| **8. Try isolated public Skopeo** | Run the generic scan with an explicitly empty authfile when earlier public mechanisms were unavailable or denied. A denial from another backend does not suppress this independent public attempt. | Same | Same | Same | Same | Same | Same | Same |
...
| **11. Offer interactive recovery**<br>Only after denial and all automatic paths | Collect a Docker Hub username/PAT and retry the Hub API, or let the user skip this input. The prompt runs only after the anonymous OCI path has also been exhausted. | Offer to refresh `gh` with `read:packages` and retry Packages, or skip this input. Anonymous OCI has already been attempted automatically. | N/A | N/A | N/A | N/A | N/A | N/A |
...
| `LOOKUP_DENIED` | Record the mechanism-specific denial and permit credentialed rows according to `--credential-policy`. Other eligible public mechanisms remain available. |

Here, **public** means that no user or configured credential is supplied; a
registry may still issue a repository-scoped anonymous bearer token. That
service-issued token does not make the attempt credentialed. Native OCI keeps
bearer headers in mode-0600 temporary files; Skopeo separates an empty public
authfile from the session authfile and receives login secrets on standard
input.
